...
import { useEffect, useRef, useState } from "react";
import { useNavigate } from "react-router-dom";
import CreateGroupModal from "../components/Chat/CreateGroupModal"; // ✅ Added
import ProfilePopup from "../components/ProfilePopup";
import api from "../services/api";
import { createSocket, disconnectSocket, getSocket } from "../services/socket";
import useAuthStore from "../store/useAuthStore";
...
export default function ChatPage() {
  ...
  const [watchId, setWatchId] = useState(null);
  const [showProfilePopup, setShowProfilePopup] = useState(false);
  const [showCreateGroup, setShowCreateGroup] = useState(false); // ✅ Added
    ...
    (async () => {
      try {
        const res = await api.get(
          `/api/auth/users?excludeId=${user?._id || ""}`
        );
        if (res.data?.ok) setContacts(res.data.users);
      } catch {}
    })();
